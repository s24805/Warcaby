...
void Player::killPiece(vector<vector<int>> &pBoard, int x, int y) {
	//spriteSheetTexture.render(x*80-40,y*80-40, &spriteClips[0]);
	//cout<<"texture h:"<<spriteSheetTexture.getHeight()<<endl;
	//pBoard[x][y]->animateDestroy(i);
    cout<<"Captured piece: \t("<<x<<" ,"<<y<<")\t"<<"from team: "<<pBoard[x][y]<<endl;
    switch (pBoard[x][y]) {
        case RED_PIECE:               
			pBoard[x][y] = RED_PIECE_D1;
            break;
        case BLACK_PIECE:               
			pBoard[x][y] = BLACK_PIECE_D1;
            break;
        case RED_KING:               
			pBoard[x][y] = RED_KING_D1;
            break;
        case BLACK_KING:               
			pBoard[x][y] = BLACK_KING_D1;
            break;
        default:                           // W przypadku domyślnym (puste pole)
            break;
    }
 // Ustawia pole na planszy jako puste
